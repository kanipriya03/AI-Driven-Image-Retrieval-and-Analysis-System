 ...
 ${f}`,f=f.replace("return",`${i} = `);let d=`
    ...
    `),t="packedUVfrom1D",o[t]=new I(`
      ...
      `),t="sampleTexture";let r=D(this.context.glContext.version);return o[t]=new I(`
        ...
        }`),o}getInputsSamplingSnippets(){let o={},t=this.context.outputTextureLayout;return this.context.programInfo.inputNames.forEach((r,n)=>{let s=this.context.inputTextureLayouts[n],i=rr(r);s.isPacked?o[i]=this.getPackedSamplerFromInput(i,r,s):o[i]=this.getUnpackedSamplerFromInput(i,r,s);let u=Ui(r);s.unpackedShape.length<=t.unpackedShape.length&&(s.isPacked?o[u]=this.getPackedSamplerAtOutputCoords(u,s,t,r):o[u]=this.getUnpackedSamplerAtOutputCoords(u,s,t,r))}),o}getPackedSamplerAtOutputCoords(o,t,r,n){let s=t.unpackedShape,i=r.unpackedShape,l=rr(n),f=s.length,p=i.length,d=it.getBroadcastDims(s,i),b=at(p),g=p-f,y,v=St();f===0?y="":p<2&&d.length>=1?y="coords = 0;":y=d.map(Mt=>`coords.${v[Mt+g]} = 0;`).join(`
